    ...
    >
      <Card className="h-full transition group-hover:-translate-y-0.5 group-hover:shadow-md" size="sm">
        {show.imageUrl === null ? (
          ...
          <img alt={m.show_poster_alt({ name: show.name })} className="aspect-[2/3] w-full object-cover" src={show.imageUrl} />
        )}
        <CardHeader>
        ...
        <CardContent className="mt-auto">
          <p className="text-muted-foreground line-clamp-3">{show.summary ?? m.show_summary_missing()}</p>
        </CardContent>
      </Card>
    </Link>
  );
...
function metadata(show: Show) {
  return [show.premiered?.slice(0, 4), show.status, show.rating === null ? null : m.show_rating({ rating: show.rating })]
