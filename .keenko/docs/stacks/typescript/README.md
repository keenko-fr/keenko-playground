...
Keenko's TypeScript tooling has two deliberate roles: the direct `typescript` package supplies the TypeScript 6 JavaScript API required by the current Nx/Oxlint integration, while `@typescript/native` supplies the TypeScript 7 compiler/typecheck path. Treat them as members of one reviewed compatibility tuple rather than collapsing or independently floating either role. TypeScript repositories use the canonical Oxfmt/Oxlint/Ultracite toolchain from `docs/core/tooling.md`, including required `oxlint-tsgolint` type-aware linting. Keep compiler verification in the canonical `typecheck` script instead of replacing it with Oxlint type checking.
